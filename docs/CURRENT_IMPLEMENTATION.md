...
v0.13仍使用Schema 6。打开旧Schema 1至5仓库时先创建校验元数据备份，再执行迁移。服务端配置、运行锁、日志和诊断包都保存在Vault控制目录中，不改变数据库Schema。
...
- 当前没有服务停止命令、Windows事件日志集成和崩溃报告；
- 当前格式可能在正式协议确定前变化。

因此v0.13适合本地开发和架构验证，不适合保存唯一副本或直接暴露公网。主密钥与整块磁盘一起被盗时，当前版本不能提供有效的离线保密保证。
...
2. Windows事件日志、服务停止命令、崩溃报告和诊断包审计；
