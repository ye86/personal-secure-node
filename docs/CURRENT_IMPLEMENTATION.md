...
v0.19使用Schema 7，新增 `share_links` 表。打开旧Schema 1至6仓库时先创建校验元数据备份，再执行迁移。服务端配置、运行锁、状态文件、日志、事件日志、诊断包和分享链接元数据都保存在Vault控制目录中；Windows同步脚本保存在同步根目录 `.psn-sync` 下。
...
- 插件中心当前仅为入口占位，尚未实现实际功能；
- 当前没有Windows事件日志集成和崩溃报告；
- 当前格式可能在正式协议确定前变化。

因此v0.19适合本地开发和局域网功能闭环验证，不适合保存唯一副本或直接暴露公网。主密钥与整块磁盘一起被盗时，当前版本不能提供有效的离线保密保证。
...
1. 插件系统雏形：插件清单、入口注册、启用禁用和权限声明；
2. 分享链接下载页美化、批量分享和上传收件箱；
