...
- `release-package` 生成Windows/Linux发布包；
- 发布包包含源码、Windows安装/卸载脚本、Linux安装/卸载脚本和发布清单；
...
v0.21继续使用Schema 8。Schema 8新增 `entities`、`artifacts`、`artifact_permissions` 和 `entity_sanctions` 表。打开旧Schema 1至7仓库时先创建校验元数据备份，再执行迁移。服务端配置、运行锁、状态文件、日志、事件日志、诊断包、分享链接元数据和插件信任元数据都保存在Vault控制目录中；Windows同步脚本保存在同步根目录 `.psn-sync` 下。
...
- 发布包仍依赖pip安装依赖，不是完整离线包；
- Windows安装脚本使用任务计划，不是签名MSI；
- Linux安装脚本使用systemd用户服务，不是deb/rpm；
- 尚无自动升级和回滚通道；
- 当前没有Windows事件日志集成和崩溃报告；
- 当前格式可能在正式协议确定前变化。

因此v0.21适合本地开发、DIY安装和局域网功能闭环验证，不适合保存唯一副本或直接暴露公网。主密钥与整块磁盘一起被盗时，当前版本不能提供有效的离线保密保证。
...
psn-drive release-package [--project-root <directory>] [--output <directory>]
...
1. 首次使用向导：设备配对、浏览器登录和复制访问地址；
2. 发布包离线依赖 wheelhouse；
3. 自动升级与回滚通道；
4. 分享链接下载页美化、批量分享和上传收件箱；
5. 插件包格式、签名校验和本地安装目录。
