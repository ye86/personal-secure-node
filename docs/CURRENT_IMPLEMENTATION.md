...
- 技术诊断信息折叠到“高级诊断”，分享链接和插件中心作为后续功能入口占位；
...
v0.18仍使用Schema 6。打开旧Schema 1至5仓库时先创建校验元数据备份，再执行迁移。服务端配置、运行锁、状态文件、日志、事件日志和诊断包都保存在Vault控制目录中；Windows同步脚本保存在同步根目录 `.psn-sync` 下，不改变数据库Schema。
...
- 分享链接和插件中心当前仅为入口占位，尚未实现实际功能；
- 当前没有Windows事件日志集成和崩溃报告；
- 当前格式可能在正式协议确定前变化。

因此v0.18适合本地开发和局域网功能闭环验证，不适合保存唯一副本或直接暴露公网。主密钥与整块磁盘一起被盗时，当前版本不能提供有效的离线保密保证。
...
1. 最小分享链接：只读下载、过期时间、下载次数和撤销；
2. 插件系统雏形：插件清单、入口注册、启用禁用和权限声明；
3. 最小发布包和安装向导，降低DIY用户部署成本；
4. Web控制台补充诊断包下载、设备列表和配对入口；
5. GUI客户端安装器、系统托盘、VSS快照和安全凭据存储。
