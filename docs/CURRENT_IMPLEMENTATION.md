...
v0.17仍使用Schema 6。打开旧Schema 1至5仓库时先创建校验元数据备份，再执行迁移。服务端配置、运行锁、状态文件、日志、事件日志和诊断包都保存在Vault控制目录中；Windows同步脚本保存在同步根目录 `.psn-sync` 下，不改变数据库Schema。
...
因此v0.17适合本地开发和局域网功能闭环验证，不适合保存唯一副本或直接暴露公网。主密钥与整块磁盘一起被盗时，当前版本不能提供有效的离线保密保证。
...
1. 最小发布包和安装向导，降低DIY用户部署成本；
2. Web控制台补充诊断包下载、设备列表和配对入口；
3. 真正的Windows服务安装器、最小权限账户和安全卸载；
4. GUI客户端安装器、系统托盘、VSS快照和安全凭据存储；
5. 灾难备份加密、自动计划、保留策略和异地复制。
