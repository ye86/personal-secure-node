...
个人数据中心首页中的文件与空间信息使用 `drive:read` 令牌。高级诊断中的状态、事件和启动预检也使用 `drive:read` 令牌；生成诊断包使用 `drive:write` 令牌。诊断包仍由服务端的脱敏逻辑生成，不包含 `.psn/master.key`、`.psn/tls.key` 或加密Blob内容，但可能包含路径、节点URL、证书指纹和日志上下文。
