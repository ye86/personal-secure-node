...
- PSN Drive HTTPS API、Web文件管理、用户侧个人数据中心、最小分享链接、插件信任模型、Windows后台同步脚本和Windows/Linux发布包。
...
# 生成不包含主密钥、TLS私钥和Blob数据的诊断包
python drive.py --vault D:\MyPsnDrive server-diagnostics

# 调试时前台运行；默认server-run会写入 .psn/logs/server.log
python drive.py --vault D:\MyPsnDrive server-run --foreground
```

`server-run` 现在会持有 `.psn/run/server.lock`，防止同一Vault启动多个服务进程。默认日志写入 `.psn/logs/server.log`，诊断包输出到 `.psn/diagnostics/`。Windows托管脚本现在也会生成 `collect-diagnostics.ps1`。详见 [服务日志、诊断包与运行锁](docs/SERVICE_OPERATIONS.md)。

### v0.14 服务预检与停止

```powershell
# 启动前检查配置、TLS、目录权限、运行状态和端口
...
`server-status` 现在会根据 `.psn/run/server.json` 中的PID判断服务进程是否仍在运行。`server-preflight` 失败时返回非零退出码，适合给安装脚本或服务包装器使用。`server-stop` 是本机PID级停止入口，还不是完整Windows服务控制管理器集成。
...
服务端现在会把启动、停止、失败、锁获取/释放、预检、停止请求、诊断包生成等事件写入 `.psn/logs/service-events.jsonl`。诊断包会包含 `logs/service-events-tail.jsonl`，便于排查服务生命周期问题，同时仍不包含主密钥、TLS私钥和Blob数据。

### v0.16 Windows客户端后台同步脚本

```powershell
# 基于已有 sync-init 配置生成后台同步脚本
python drive.py windows-sync-scripts D:\PsnDevice\pictures-sync.json

# 自定义任务名和同步间隔
python drive.py windows-sync-scripts D:\PsnDevice\pictures-sync.json `
  --task-name PSNDrivePictures --interval 300
```

默认会在同步根目录的 `.psn-sync\service\windows\` 生成 `sync-watch.ps1`、`sync-run-once.ps1`、`sync-status.ps1`、任务计划安装/卸载脚本。它还不是GUI安装器，但已经能让 Windows 文件夹备份以任务计划方式后台运行。详见 [Windows文件夹同步客户端](docs/WINDOWS_SYNC.md)。

### v0.17 Web管理控制台

启动节点服务并在浏览器打开 `https://127.0.0.1:7780/` 后，使用 `device-login` 返回的短期令牌连接。Web页面现在包含管理控制台，可查看：

- 服务运行状态、PID、节点地址和服务名；
- 服务日志与事件日志大小；
- Vault逻辑数据量、分块数和回收站文件数；
- 最近服务生命周期事件；
- 启动预检结果；
- 一键生成不含主密钥、TLS私钥和Blob数据的诊断包。

控制台仍是局域网/本机管理原型，不包含停止服务、修改配置或公网管理能力。详见 [Web文件管理界面](docs/WEB_UI.md)。

### v0.18 用户侧个人数据中心

Web首页现在优先面向普通用户，而不是传统IT管理后台。连接节点后首先看到：

- 个人数据中心首页；
- 我的文件、已用空间、备份状态和回收站摘要；
- 上传文件、查看文件、打开回收站、导出问题报告等常用操作；
- 最近文件列表；
- 分享链接和插件中心的产品入口占位；
- 折叠到“高级诊断”里的服务状态、PID、日志、预检和事件详情。

v0.18的重点是产品表达：先让DIY用户和普通用户看得懂、愿意用。插件系统和插件市场仍在后续版本实现。

### v0.19 最小分享链接

Web文件列表现在可以给单个文件创建只读下载链接。创建时可设置：

- 有效天数；
- 最多下载次数；
- 随时撤销。

分享链接固定到创建时的文件版本，因此你之后更新同名文件，不会改变已经发出去的下载内容。被分享的人不需要账号，访问 `/s/<token>` 即可下载。这个版本只负责在节点当前可访问地址上提供下载，不包含公网穿透、动态域名、反向代理配置或外部分享页美化。

### v0.20 插件主体与权限管理雏形

Web界面新增“插件中心”。当前版本不执行第三方代码，而是先登记插件和主体关系：
...
这个模型也将作为未来AI Skills的底层权限模型，避免插件市场和AI助手各自发明一套授权体系。详见 [插件、主体与未来AI Skills权限模型](docs/PLUGIN_TRUST_MODEL.md)。

### v0.21 Windows/Linux发布包

```powershell
python drive.py release-package --output dist
```

该命令生成 `psn-drive-<version>.zip`，包含发布包内源码、Windows安装脚本、Linux安装脚本、卸载脚本和发布说明。安装脚本会创建Python虚拟环境、安装PSN Drive、初始化Vault、生成服务配置并注册后台运行。

它不是MSI/deb/rpm，但已经适合早期DIY用户下载解压后一键部署。详见 [Windows与Linux发布包部署](docs/RELEASE_DEPLOYMENT.md)。
...
- [Windows与Linux发布包部署](docs/RELEASE_DEPLOYMENT.md)
...
- [服务日志、诊断包与运行锁](docs/SERVICE_OPERATIONS.md)
